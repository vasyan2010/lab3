...
class haf_Tree{   
	...
	public:
		...
		void makeTree (char input[], int sizeInput)
		{
			...
			{
				if (nodeHead->frequency<(nodeHead->next->frequency)) //если первый меньше второго
				{
			...
			cout<<"Memory saved: "<<sizeInput*8<<" - "<<encryptedStringSize<<" = "<<sizeInput*8-encryptedStringSize<<endl;		
		}		
